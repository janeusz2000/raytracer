...
def main():
    ...
    color_que = multiprocessing.SimpleQueue()
    ...
    triangle2 = TriangleFace(Color(1, 0, 0), Vector(1, 0, 1), Vector(1, 1, 1), Vector(2, 0, 1))
    viewer.add_object(triangle2)
    ...
    process_list = [multiprocessing.Process(target=gui_loop, args=(gui,)).start()]

    for a in range(4):
        process_list.append(multiprocessing.Process(target=core_loop))
